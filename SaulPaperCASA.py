...
if __name__ == '__main__': #This only runs is we run directly but not if you try to import it.
    ...
    for msname in msname_:
        ...
        clean(msname,msname[:-5]+'_KGcal_model',niter=0, weighting = 'briggs',robust =0,
              imsize =[512 ,512] ,pbcor=False,phasecenter = 'J2000 17h45m40.0409s -29d0m28.118s',
              cell=['500 arcsec'],mode='mfs',nterms=1, spw='0:150~900', stokes='IQUV',
        ...
        clean(msnamei,msnamei[:-5]+'_KBcal_NoDeconv',niter=0, weighting = 'briggs',robust =0,
        ...
        exportfits(msnamei[:-5]+'_KBcal_NoDeconv.image',msnamei[:-5]+'_KBcal.image.fits')

        # Make Interactive CLEAN image of Simulated data and export to fits
        imgnameC = msname[:-5]+'_interactive'
        fitsimgC = msname[:-5]+'_interactive.image.fits'

        clean(msname,imgnameC,niter=0, weighting = 'briggs',robust =0,
              imsize =[512 ,512] ,pbcor=False,
        ...
        fitsname = 'mult_spec_chan_1024_'+msnamei[:-3]+'.image.fits'
        imagename = 'mult_spec_chan_1024_'+msnamei[:-3]+'.image'
    
        clean(msnamei,imagename[:-6],niter=0,weighting = 'briggs',robust =0,imsize =[512 ,512]
              ,cell=['500 arcsec'],phasecenter = 'J2000 17h45m40.0409s -29d0m28.118s'
